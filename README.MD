...
This repo is **~11GB**: 101,698 prediction npz files plus chunked image galleries
and bottleneck caches. How you clone depends on what you need.

**Just the code** (measured: **95 seconds, 14MB on disk**):
```bash
git clone --filter=blob:none --no-checkout --depth 1 https://github.com/aloshdenny/e85.git
cd e85
git sparse-checkout set scripts README.MD requirements.txt
git checkout main
```
`--no-checkout` matters. `--filter=blob:none` on its own still checks out every
file, so git lazily fetches all 107k blobs anyway -- measured at 435MB and still
running after 40 minutes. Skipping the checkout and sparse-setting first is what
makes it fast.

**Everything, including the data** (slow, plan for it):
```bash
...
`requirements.txt` pins the full working environment, including `tribev2` itself
from GitHub. Nothing else needs fetching by hand: the figure scripts download
TRIBE's 709MB `best.ckpt` from the HuggingFace Hub on first use and cache it.

> **Chunked artifacts**: anything over GitHub's 100MB limit is committed as
> `{stem}_chunk_NNN{ext}` parts, so the fused file **does not exist** after a
> clone. Always go through `scripts/chunk_utils.py` (`load_npz`, `npz_exists`,
> `save_npz`, `load_chunked`, `save_chunked`, `ensure_fused_zip`,
> `chunk_file_if_needed`) rather than a bare `np.load` / `torch.load` /
> `Path.exists`, which silently find nothing. `verify_reproducible.py` enforces
> this.
...
**Mia**
![Mia brain maps](abliterated/brain_maps/mia_brain_maps.png)

**Sins**
![Sins brain maps](abliterated/brain_maps/sins_brain_maps.png)

**Michael Jackson**
![Michael Jackson brain maps](abliterated/brain_maps/michael_jackson_brain_maps.png)
...
The post-surgery activity figure needs TRIBE's readout matrix, which lives inside
the 709MB `best.ckpt`. The script fetches it from the HuggingFace Hub on first run
and reuses the cached copy, so no manual download step:

```bash
python scripts/plot_postop_activity.py                    # fetches best.ckpt
python scripts/plot_postop_activity.py --ckpt best.ckpt   # or use a local copy
python scripts/plot_suppression_brain_maps.py
python scripts/plot_holdout_split.py
```

All paths in these scripts resolve relative to the repo, so they run from any
working directory on any machine.

### Reproducibility check

Chunked artifacts do **not** exist under their logical name in a fresh clone --
only the `_chunk_NNN` parts do. So any code touching them must go through
`chunk_utils` (`load_npz` / `npz_exists` / `save_npz` / `load_chunked` /
`save_chunked` / `ensure_fused_zip`), never a bare `np.load` or `path.exists()`,
which would fail or silently skip the data.

`scripts/verify_reproducible.py` enforces this: it checks nothing committed
exceeds GitHub's 100MB limit, that every chunk set actually re-fuses and loads,
and that no script reaches for a chunked artifact with a raw loader. Run it
before pushing new artifacts.

```bash
python scripts/verify_reproducible.py   # exit 0 = a fresh clone will run
```

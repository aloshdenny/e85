...
def main():
    ...
    for n, (img, is_t) in enumerate(imgs):
        ...
        # Upsampling doubles the image before scanning, which only matters for
        # small faces; on 1024px FFHQ frames it is pure cost.
        faces = det(gray, 0 if min(gray.shape[:2]) >= 300 else 1)
        if not faces:
            skipped += 1
            continue
        pts = np.array([[p.x, p.y] for p in sp(gray, faces[0]).parts()], dtype=np.float32)

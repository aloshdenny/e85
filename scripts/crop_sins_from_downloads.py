"""Crop and upright-align Johnny Sins faces from today's Downloads.

Discovers images modified today under Downloads/ and Downloads/Screenshots/,
clusters InsightFace embeddings so podcast/cameo faces are dropped, rotates
each keep so the eyes sit on a horizontal line (fixes lopsided screenshots),
and writes square 256px crops.

Usage:
  python scripts/crop_sins_from_downloads.py
"""
...
def todays_images():
    out = []
    for root, max_depth in ((DOWNLOADS, 1), (SCREENSHOTS, 3)):
        if not root.exists():
            continue
        for p in root.rglob("*"):
            ...
            if len(rel.parts) > max_depth:
                continue
            if root == DOWNLOADS and p.parent != DOWNLOADS:
                continue
...
def upright_square(img, bbox, kps, out_size=OUT_SIZE, margin=MARGIN):
    ...
    Padding first so a tilted bald head is not clipped. This is more stable on
    3/4 views than warping onto the 112px ArcFace template.
    """
...
def main():
    ...
    paths = todays_images()
    print(f"{len(paths)} images from {TODAY}")
    if not paths:
        raise SystemExit("no images today")

    app = FaceAnalysis(name="buffalo_l", providers=["CPUExecutionProvider"])
    app.prepare(ctx_id=-1, det_size=(640, 640))

    detections = []  # (path, face, emb)
    for p in paths:
        img = load_bgr(p)
        if img is None:
    ...
    # majority identity = embedding with most neighbors above CLUSTER_SIM
    S = E @ E.T
    ...
    for i, (p, img, f, emb) in enumerate(detections):
        ...
        if keep_mask[i]:
            kept += 1
            save_jpg(OUT_DIR / f"{stem}_sins{kept:02d}.jpg", crop)
        else:
            rejected += 1
